const getNotification = (userId, callBack) => {
  let sql = `SELECT notification.id, type, date, username, picture1 FROM notification INNER JOIN user ON notification.id_user1 = user.id WHERE notification.id_user2 = ${db.escape(userId)} ORDER BY date DESC`;
...
const countNotification = (userId, callBack) => {
  let sql = `SELECT COUNT(*) AS notifNumber FROM notification WHERE id_user2 = ${db.escape(userId)} AND readed = 0`;
  db.query(sql, (err, result, data) => {
    if (err) throw err;
    if (result.length > 0) {
      return callBack(null, result, data);
...
const readNotification = (userId, callBack) => {
  let sql = `UPDATE notification SET readed = 1 WHERE id_user2 = ${db.escape(userId)} AND readed = 0`;
  db.query(sql, (err) => {
    if (err) throw err;
